...
function LoginForm() {
    ...
    return html`
        <form class="access-form" @submit=${onSubmit}>
            <input id="email-input" type="email" name="email" placeholder="${translate("loginForm.placeholder")}" autocomplete="email" required .value=${email} .disabled=${fetching} @input=${onEmailInput}>
            <button .disabled=${fetching}>${translate("loginForm.btn")}</button>
        </form>
        <h3>${translate("loginForm.subheading")}</h3>
        <div class="oauth-providers">
            <a class="btn" href="${location.origin}/api/github_auth?redirect_uri=${encodeURIComponent(location.origin + "/access-callback")}" data-default="true">
