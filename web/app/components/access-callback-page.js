...
function AccessCallbackPage() {
    ...
    const [retryEndpoint, setRetryEndpoint] = useState(null)
    ...
    useEffect(() => {
        ...
        if (data.has("error")) {
            ...
            if (data.has("retry_endpoint") && (err.name === "UserNotFoundError" || err.name === "InvalidUsernameError" || err.name === "UsernameTakenError")) {
    ...
    return html`
        <main class="container">
            ...
            ${err !== null ? html`
                <p class="error" role="alert">Error: ${err.message}</p>
                ${err.name === "MissingAuthDataError" ? html`
                    <a href="/">Go home</a>
                ` : nothing}
            ` : nothing}
            ${retryEndpoint !== null ? html`
                <form class="username-form" @submit=${onUsernameFormSubmit}>
                    <input type="text" name="username" placeholder="Username" pattern="^[a-zA-Z][a-zA-Z0-9_-]{0,17}$" autofocus .value=${username} @input=${onUsernameInput}>
